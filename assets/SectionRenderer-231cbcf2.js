...
`)),r.toLowerCase().endsWith(".vue")){let n=y.parse(e),t=y.stringify(n,!1,!0,!0),s=t.style;s=s.replaceAll("<style>","").replaceAll("<style scoped>","").replaceAll("</style>",""),s=await i(s,/(?:url\()(.*?)(?=\))/g,async(d,l)=>{let p=await a(l,!1,o);return d.replace(l,p)}),s=await i(s,/(?:url\(')(.*?)(?='\))/g,async(d,l)=>{let p=await a(l,!1,o);return d.replace(l,p)}),s=await i(s,/(?:url\(")(.*?)(?="\))/g,async(d,l)=>{let p=await a(l,!1,o);return d.replace(l,p)}),s=s.replaceAll("@blob","blob");let c=URL.createObjectURL(new Blob([s],{type:"text/css"})),u=t.template.replaceAll(`
`,"").trim();u=await i(u,/(?:\=")(.*?)(?=")/g,async(d,l)=>{let p=await a(l,!1,o);return d.replace(l,p)}),u=await i(u,/(?:url\()(.*?)(?=\))/g,async(d,l)=>{let p=await a(l,!1,o);return d.replace(l,p)}),u=await i(u,/(?:url\(')(.*?)(?='\))/g,async(d,l)=>{let p=await a(l,!1,o);return d.replace(l,p)}),u=await i(u,/(?:url\(")(.*?)(?="\))/g,async(d,l)=>{let p=await a(l,!1,o);return d.replace(l,p)}),u=u.replaceAll("@blob","blob"),e=t.script.replace("export default{",`
...
export default{
template: \``+u+"`,"),e=e.replaceAll("this.router","window.router"),["setup"].forEach(d=>e=e.replace("<script "+d+">","")),e=e.replace("<script>",""),e=e.replace("<\/script>","")}return URL.createObjectURL(new Blob([e],{type:"text/javascript"}))},i=async(e,r,o)=>{let n=[];e.replace(r,(s,...c)=>{const u=o(s,...c);n.push(u)});const t=await Promise.all(n);return e.replace(r,()=>t.shift())},N=()=>R(),R=()=>{h={},h.vue=URL.createObjectURL(new Blob([P],{type:"text/javascript"})),h["vue-router"]=URL.createObjectURL(new Blob([L.replace("from 'vue'",`from '${h.vue}'`)],{type:"text/javascript"}))};R();let O=async(e,r)=>{f=e,r()};const I=`let ORIGIN = origin || window.origin
let SELECTED = null
...
let selectRoot = () => {
  ...
  window.parent.postMessage({event: 'click', target}, origin)
      ...
      `,n=this.code.script||"";n=n.replace("<script>",""),n.indexOf("export default{")>=0&&(n=n.replace("export default{",`
          ${e}
          let APP = { ${r},
        `),n=n.slice(0,n.lastIndexOf("}"))+"}");let t=this.mockup(),s=await a("/src/main.js",!0,[this.path]),c=await(await fetch(s)).text();return t&&(c=`import mockup from '${t}'
`+c),c=c.replace(/({[\s\S]*?createApp)/,"$1, reactive, compile, h, isVNode"),c=c.replace("import App","// import App"),c=c.replace("const app = createApp(App)",`
      ...
      `),c},async setStyle(e){e=await i(e,/(?:url\()(.*?)(?=\))/g,async(r,o)=>{let n=await a(o);return r.replace(o,n)}),e=await i(e,/(?:url\(')(.*?)(?='\))/g,async(r,o)=>{let n=await a(o);return r.replace(o,n)}),e=await i(e,/(?:url\(")(.*?)(?="\))/g,async(r,o)=>{let n=await a(o);return r.replace(o,n)}),this.css=e.replaceAll("@blob","blob")},async setTemplate(e){e=await i(e,/(?:\=")(.*?)(?=")/g,async(r,o)=>{let n=await a(o);return r.replace(o,n)}),e=e.replaceAll("@blob","blob"),this.events||(e=e.replaceAll("@click","@click-disabled")),this.template="",this.$nextTick(()=>this.template=e)},async addModule(e){let r=document.createElement("script");return r.type="text/javascript",e=e.trim(),["setup"].forEach(n=>e=e.replace("<script "+n+">","")),e=e.replace("<script>","").replace("<\/script>",""),r.setAttribute("type","module"),r.src=await S(e),r},addMap(e){let r=document.createElement("script");return r.setAttribute("type","importmap"),r.innerHTML=e.trim(),r},addScript(e){let r=document.createElement("script");return r.innerHTML=e.trim(),r},addStyle(e){let r=document.createElement("hgroup");return r.innerHTML=e,r},async renderIframe(){if(this.rendering)setTimeout(()=>this.rendering?"":this.renderIframe(),500);else{this.rendering=!0,N();let e=await this.master(),r=await this.addModule(e+`

          let origin = '${window.origin}'
          ${I}
        `),o=document.getElementById("iframe-container"),n=document.createElement("iframe");n.setAttribute("id","builder"),o.innerHTML="",o.appendChild(n),navigator.userAgent.toLowerCase().includes("firefox")?n.onload=async()=>await this.iframeInit(n,r):await this.iframeInit(n,r),setTimeout(()=>this.rendering=!1,500)}},clear(){let e=document.getElementById("iframe-container");e.innerHTML=""},async iframeInit(e,r){try{const o=document.createElement("script");o.src="https://cdn.tailwindcss.com",e.contentDocument.head.appendChild(o);let n=JSON.parse(await this.fs.readFile("/package.json",{encoding:"utf8"})),t={imports:{}};for(const s of Object.keys(n.dependencies)){const c=n.dependencies[s];c.startsWith("http")||c.startsWith("git")?t.imports[s]=c:s==="vue"?t.imports[s]=await a("vue"):s==="vue-router"?t.imports[s]=await a("vue-router"):s==="template-compiler"?t.imports[s]=await a("template-compiler"):t.imports[s]="https://cdn.jsdelivr.net/npm/"+s+"@"+c+"/+esm"}e.contentDocument.head.appendChild(this.addMap(JSON.stringify(t))),e.contentDocument.body.innerHTML=`
          <div id="app"></div>
